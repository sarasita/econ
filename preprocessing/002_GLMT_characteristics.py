...
if __name__ == '__main__': 
    ...
    for i_scen in range(cset.n_scenarios): 
        ...
        glmt[i_scen*100:(i_scen+1)*100, :] = xr.load_dataset(Path.joinpath(cset.path_MESMER_fldmean, f"{scenario}_fldmean.nc"))['tas'].values[:, -86:]
        columns.append([scenario + f'_{i_run}' for i_run in range(100)])
    columns = np.array(columns).flatten()
    
    cset.path_MESMER_char.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(glmt.T, index = np.linspace(cset.year_start,cset.year_stop,cset.n_years), columns = columns).to_csv(Path.joinpath(cset.path_MESMER_char, f'glmt_dataset.csv'))
    ...
    cset.path_MESMER_char.mkdir(parents=True, exist_ok=True)
    glmt_char_df.to_csv(Path.joinpath(cset.path_MESMER_char, f'glmt_characteristics_thsld_{int(glmt_threshold*100)}.csv'))
